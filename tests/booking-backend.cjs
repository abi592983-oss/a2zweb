...
accepting=true;
const pull=context.doGet({parameter:{action:'erp_pull',token:'test-sync-secret',limit:'100'}});
...
assert.equal(context.doGet({parameter:{action:'erp_pull',token:'test-sync-secret'}}).rows.length,6);
