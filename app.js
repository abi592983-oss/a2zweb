...
function applyBrand() {
  ...
  if (footerContact) footerContact.innerHTML = `<p>${esc(data.address)}</p><div class="actions">${contacts()}</div>`;
...
function healthNotice(){return `<div class="notice"><p><strong>Limited health-check slots per day. Your slot is confirmed only after we contact you.</strong></p><p>Only working computers, laptops and CCTV systems qualify. Faulty or non-working devices need a repair appointment.</p>${link('#booking','Book a repair appointment','outline')}</div>`}
function booking(){return intro('Appointment booking','Book an appointment','For repairs, installation and networking. We’ll call to confirm.')+`<div class="booking-layout">${A2ZBooking.render(data)}<p class="note">LKR ${esc(data.deposit)} booking deposit · Pay after confirmation.</p><p>${link('#health','Book a health check','outline')}</p><div class="actions">${contacts()}<a href="${safeURL(data.booking?.formUrl)}" class="textlink">Alternative: open Google Forms ↗</a></div></div>`}
function healthBooking(service){
  if(!['computer_health','cctv_health'].includes(service))return notfound();
  const title=service==='computer_health'?'Computer / laptop health check':'CCTV health check';
  return `<section class="wrap">${intro('Health-check booking',title,'Your health-check type is selected. Enter your contact details below.')}<div class="booking-layout">${healthNotice()}${A2ZBooking.render(data,service)}<p class="note">LKR ${esc(data.deposit)} booking deposit · Pay after confirmation.</p><div class="actions">${contacts()}<a class="textlink" href="#health">Choose a different health check</a></div></div></section>`;
}

function renderContent(){const route=location.hash.slice(1)||'home';let html='';
if(route==='home'){html=`<section class="wrap hero"><div><span class="eyebrow">YOUR NEIGHBOURHOOD TECH PARTNER</span><h1>${esc(data.hero.title)}</h1><p>${esc(data.hero.description)}</p><div class="actions">${link('#booking','Book an appointment')}${link('#health','Book a health check','outline')}</div></div><div class="hero-photo"><img src="${safeURL(data.hero.image)}" alt="Close-up of computer motherboard components" fetchpriority="high"><div class="photo-note"><div><small>FROM THE WORKBENCH TO YOUR WORKDAY</small><strong>We keep you connected.</strong></div><span aria-hidden="true">↗</span></div></div></section><div class="trust"><span>✓ NVQ-qualified technician</span><span>✓ Local workshop</span><span>✓ On-site service by arrangement</span></div><section class="wrap"><div class="sectionhead"><div><span class="eyebrow">WHAT CAN WE HELP WITH?</span><h2>Your tech. Our everyday.</h2></div><a class="textlink" href="#services">Explore services ↗</a></div>${cards()}</section><section class="wrap" style="padding-top:0">${health()}</section>`;}
...
else if(route==='health'){html=`<section class="wrap">${intro('Health-check booking','Book a health check','Choose your working computer or CCTV system.')}${healthNotice()}<div class="columns">${data.healthChecks.map(t=>`<div class="panel"><span class="badge">HEALTH CHECK</span><h3>${esc(t)}</h3><p>${t.startsWith('CCTV')?'Review camera coverage, recording and overall system operation.':'Review storage health, temperatures, system errors and battery condition where supported.'}</p>${link(t.startsWith('CCTV')?'#health-booking/cctv_health':'#health-booking/computer_health','Book this health check')}</div>`).join('')}</div><div class="cta"><h2>Findings you can use.</h2><p>Receive an explanation of the checks performed and recommended next steps. Available checks depend on the device.</p><p class="notice">Device already faulty? Choose a repair booking for diagnosis.</p></div></section>`}
else if(route.startsWith('health-booking/')){html=healthBooking(route.split('/')[1]);}
else if(route==='health-booking'){html=healthBooking('');}
