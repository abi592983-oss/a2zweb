...
function booking(){return intro('LET’S GET IT SORTED','Tell us what you need.','Complete the booking form. A visit is confirmed only after we contact you.')+`<div class="columns">${A2ZBooking.render(data)}<aside class="panel"><span class="badge">BY APPOINTMENT</span><h3>One request.<br>A clear next step.</h3><p>1. Tell us about the device or installation.</p><p>2. We confirm the work, location and appointment.</p><p>3. Pay the booking deposit once arrangements are confirmed.</p><hr><p><strong>LKR ${esc(data.deposit)} booking deposit</strong><br>Do not send payment before your appointment is confirmed.</p><p class="note">Health checks cover working systems. Fault diagnosis is a separate service.</p><a href="${safeURL(data.booking?.formUrl)}" class="textlink">Alternative: open Google Forms ↗</a></aside></div>`}
